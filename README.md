...
브라우저에서 `http://127.0.0.1:5173`을 연다. Web Serial은 지원되는 Chrome/Edge의 보안 컨텍스트에서 사용해야 하며 포트 선택은 학생이 연결 버튼을 눌렀을 때만 요청한다. 로컬 비식별 저장 API는 `http://127.0.0.1:8787`에서 함께 실행된다.
